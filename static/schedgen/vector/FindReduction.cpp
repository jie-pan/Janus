...
insertInitRules(Loop *loop, bool isMultiply, VariableState *phiNode, RegSet &freeRegs, RegSet &freeVectRegs,
    ...
    for (auto initBid: loop->init) {
        ...
        MachineInstruction *instr = &(bb->instrs[bb->size-1]);//the last instr of basic block
...
insertReduceRules(Loop *loop, bool isMultiply, VariableState *phiNode, RegSet &freeVectRegs, 
    ...
    for (auto bid: loop->exit) {
        ...
        rules.insert(new VECT_REDUCE_AFTER_rule(bb, bb->instrs[0].pc, bb->instrs[0].id, //the first instr of basic block
...
foundReductionVar(Loop *loop, VariableState *phiNode, set<uint32_t> &reductRegs,
    ...
    insertInitRules(loop, isMultiply, phiNode, freeRegs, freeVectRegs, rules);//init
    insertReduceRules(loop, isMultiply, phiNode, freeVectRegs, rules);//after
