...
static bool finalValueAnalysis(Loop *loop)
{
    ...
    for (auto checkID: loop->check) {
        ...
        if (!cmpInstr) {
            LOOPLOG("\t\tCould not locate an instruction that modifies control flag! "<<endl);
            return false;
        }
        ...
        if (cmpInstr->opcode != Instruction::Compare) {
            ...
            if (cmpInstr->opcode == Instruction::Sub) {
                LOOPLOG("\t\tFound a subtraction instruction that modifies eflags! "<<*cmpInstr<<endl);
            }
            else {
                LOOPLOG("\t\tFound irregular instruction that modifies eflags! "<<*cmpInstr<<endl);
                return false;
            }
        }

        if (cmpInstr->inputs.size() != 2) {
            LOOPLOG("\t\tFound irregular compare instruction with less or more than two operands! "<<*cmpInstr<<endl);
            return false;
        }
        ...
        for (auto &it : iterators) {
            if (boundExpr.hasTerm(it.first)) {
                ...
                if (!cmpOP1->hasTerm(it.first))
                    miter->checkState = loop->getAbsoluteStorage(cmpInstr->inputs[0]);
                if (!cmpOP2->hasTerm(it.first)) {
                    if (miter->checkState) {
                        LOOPLOG("\t\tLoop contains irregular comparisons! "<<endl);
                        miter->checkState = NULL;
                    } else {
                        miter->checkState = loop->getAbsoluteStorage(cmpInstr->inputs[1]);
                    }
                }
            }
        }
        ...
        if (!miter) {
            LOOPLOG("\t\tCould not find the main iterator! - Try expanding the expression to variable scope "<<endl);
            //check the expressions again
            for (auto &it : iterators) {
                Variable itVar = *(Variable *)it.first;
                ExpandedExpr boundTest = boundExpr;
                bool found = boundTest.extendToVariableScope(loop->parent, loop, &itVar);
        ...
        //get the last value of this iterator based on the compare and jump instruction
        //for the moment, we list those cases one by one
        //Note: this is not architectural independent
        //XXX: improve this in the future
        //cjump->print(&cout);
        //check which cjump target or fall through target is outside of the loop
...
bool iteratorAnalysis(Loop *loop)
{
    ...
    if (!finalValueAnalysis(loop)) return false;

    //till now, the iterator has been identified
    if (miter) {
        LOOPLOG("\t\tIterator identified: "<<(Variable)*miter->vs<<" "<<*miter<<endl);
        miter->main = true;
    }
...
bool postIteratorAnalysis(janus::Loop *loop)
{
    ...
    if (!miter) {
        loop->unsafe = true;
        LOOPLOG("\tFurther analysis disabled due to failed iterator analysis"<<endl);
        return false;
    }

    LOOPLOG("\tLoop "<<dec<<loop->id<<" post iterator "<<*miter<<endl);
        ...
        if (initExprs && finalExprs &&
            ...
            *miter->stepExprs = spectrum;
            ...
            if (pair.first == Expr::INTEGER) {
                ...
                loop->staticIterCount = miter->step;
                LOOPLOG("\t\tTotal iteration count: "<<dec<<miter->step<<endl);
            } else {
                LOOPLOG("\t\tTotal iteration count: ("<<spectrum<< ")/"<<miter->stride<<endl);
                miter->stepKind = Iterator::CONSTANT_EXPR;
                *miter->stepExprs = spectrum;
            }
    ...
    LOOPLOG("\t\tLoop "<<dec<<loop->id<<" improved iterator "<<*miter<<endl);
...
static void getIteratorFinalValue(ExpandedExpr &boundExpr, Iterator &iterator)
{
    ...
    if (!boundExpr.hasTerm(iterator.vs)) return;
    ...
    if (boundExpr.size()==2) {
        ...
        } else LOOPLOG("\t\tNot able to solve expression:"<<boundExpr<<endl);
    } else if (A.i == 1) {
...
{
    if (initKind == INTEGER) return Expr(initImm);
    else if (initKind == SINGLE_VAR) return Expr(initVar);
    else if (initKind == EXPR) return initExpr;
    else return Expr(initExprs);
}
...
void
encodeIterators(Loop *loop)
{
    LOOPLOG("\tEncoding recognised variables into portable variable profiles"<<endl);
    JVarProfile profile;
        ...
        if (iter.kind == Iterator::INDUCTION_IMM || iter.kind == Iterator::INDUCTION_VAR) {
            ...
            if (iter.main) {
                ...
                if (iter.checkState){
                    ...
                    if (!iter.finalExpr.vs) continue;
                    profile.induction.check = *iter.finalVar;
                } else {
                    if (iter.finalExprs) {
                        LOOPLOG("\t\t\tIrregular Check "<<*iter.finalExprs<<endl);
                    }
            ...
            //if both the check and stride variables are constant, encode the init value too
            if (iter.kind == Iterator::INDUCTION_IMM &&
            ...
            } else profile.induction.init.type = JVAR_UNKOWN;
...
{
    if (!vs || !vs->isPHI) return NULL;
    for (auto pred: vs->pred)
        if (pred && pred->lastModified && pred->lastModified->opcode == Instruction::Add)
