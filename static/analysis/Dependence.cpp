...
void
dependenceAnalysis(janus::Loop *loop)
{
    if (!loop) return;
    if (loop->dependenceAvailable) return;
    //skip loops with two init blocks
    if (loop->init.size()>1) return;
    ...
    for (auto phi:loop->start->phiNodes) {
        ...
        if (status == FoundCyclic) {
            ...
            LOOPLOG("\t\tDepending phi node "<<phi<<" cycle not found" <<endl);
                ...
                set<Expr *>     &visitedPhi)
{
    if (!currentExpr) {
        ...
        if (!startExpr) return Error;

        if (startExpr->kind != Expr::PHI) {
            LOOPLOG("\t\t\tThe start expression for buildCyclicExpr must be a PHI expression !" <<endl);
            return Error;
        }

        startExpr->expandedCyclicForm = NULL;
        //the phi expression should have two upstream paths
        //one path must be a cycle and the other one must be initial values from the function entry
        ExpandedExpr *e1 = new ExpandedExpr(ExpandedExpr::SUM);
        ExpandedExpr *e2 = new ExpandedExpr(ExpandedExpr::SUM);

        CyclicStatus r1 = buildCyclicExpr(startExpr, startExpr->p.e1, e1, loop, visitedPhi);
        CyclicStatus r2 = buildCyclicExpr(startExpr, startExpr->p.e2, e2, loop, visitedPhi);

        if (r1 == Error || r2 == Error) {
            LOOPLOG("\t\t\tNo cycle is found for phi node: "<<startExpr->vs <<" (traversal error)!" <<endl);
            return Error;
        }

        if (r1 == FoundCyclic) {
            if (e1->isEmpty()) return FoundConstPhi;
        ...
        if (r2 == FoundCyclic) {
            if (e2->isEmpty()) return FoundConstPhi;
        ...
        if (r1 == FoundUndecidedPhi || r2 == FoundUndecidedPhi) {
            LOOPLOG("\t\t\tNo cycle is found for phi node: "<<startExpr->vs <<"!" <<endl);
            delete e1;
            delete e2;
            return FoundUndecidedPhi;
        }

        if (r1 == FoundCyclic || r2 == FoundCyclic) return FoundCyclic;
        else if (r1 == FoundConstPhi || r2 == FoundConstPhi) {
            return FoundConstPhi;
        } else return FoundConst;
        ...
        switch (currentExpr->kind) {
            ...
            case Expr::UNARY:
                if (currentExpr->u.op == Expr::MOV) {
                    return buildCyclicExpr(startExpr, currentExpr->u.e, expr, loop, visitedPhi);
                } else if (currentExpr->u.op == Expr::NEG) {
                    ExpandedExpr negExpr(ExpandedExpr::SUM);
                    CyclicStatus result = buildCyclicExpr(startExpr, currentExpr->u.e, &negExpr, loop, visitedPhi);
                    negExpr.negate();
                    expr->merge(negExpr);
                    return result;
                } else {
            ...
            case Expr::BINARY: {
                ...
                if (currentExpr->b.op == Expr::ADD || currentExpr->b.op == Expr::MUL) {
                    r1 = buildCyclicExpr(startExpr, currentExpr->b.e1, expr, loop, visitedPhi);
                    r2 = buildCyclicExpr(startExpr, currentExpr->b.e2, expr, loop, visitedPhi);
                } else if (currentExpr->b.op == Expr::SUB) {
                    r1 = buildCyclicExpr(startExpr, currentExpr->b.e1, expr, loop, visitedPhi);
                    ExpandedExpr negExpr(ExpandedExpr::SUM);
                    r2 = buildCyclicExpr(startExpr, currentExpr->b.e2, &negExpr, loop, visitedPhi);
                    negExpr.negate();
                    expr->merge(negExpr);
                } else {
            ...
            case Expr::PHI: {
                ...
                if (currentExpr->vs &&
                    func->iterators.find(currentExpr->vs) != func->iterators.end())
                {
                    ...
                    //if the loop is the descendants, use the final value of the iterator
                    if (loop->descendants.find(suspect) != loop->descendants.end()) {
                        LOOPLOG("\t\t\tFound phi iterator in its sub loop "<<dec<<suspect->id<<endl);
                        ...
                        if (sit->finalKind == Iterator::INTEGER) {
                            ...
                            for (auto term: sit->finalExprs->exprs) {
                                ...
                                if (t.kind == Expr::INTEGER) {
                                    ...
                                    CyclicStatus status2 = buildCyclicExpr(startExpr, &t, expr, loop, visitedPhi);
                                    if (status2 == FoundCyclic) status = FoundCyclic;
                                    else if (status2 == FoundConst || status2 == FoundConstPhi) continue;
                                    else return status2;
                                }
                            ...
                            LOOPLOG("\t\t\tFinal value for the iterator not found "<<dec<<suspect->id<<endl);
                            return FoundUndecidedPhi;
                        }
                    }
                    ...
                    /* If we encounter a phi node when building cyclic expression for a depending variable
                     * it means that the variable is updated with some conditions
                     * In a loop environment, there could be internal cycles in the SSA where phi nodes are referencing each other 
                     * we should avoid checking those cycles but associate the expression with a condition */
                    ExpandedExpr *ep1 = new ExpandedExpr(ExpandedExpr::SUM);
                    CyclicStatus r1 = buildCyclicExpr(startExpr, currentExpr->p.e1, ep1, loop, visitedPhi);
                    ...
                    if (r1==FoundCyclic && r2==FoundCyclic) {
                         if (*ep1 == *ep2) {
                            ...
                            LOOPLOG("\t\t\tFound an iterator with phi expressions in stride, currently not handled "<<currentExpr->vs<<endl);
                            return FoundUndecidedPhi;
                         }
                    } else if (ep1->isEmpty() && ep2->isEmpty()) {
                        //if both expressions are empty, then it is a fake phi node created by compiler code generation
                        //it means no changes made per iteration
                        LOOPLOG("\t\t\tFound empty cyclic expressions in stride: "<<currentExpr->vs <<" no actual dependency"<<endl);
                        loop->constPhiVars.insert(currentExpr->vs);
                        return FoundConstPhi;
                    } else {
                        LOOPLOG("\t\t\tFound phi node in the iterator stride, currently not handled "<<currentExpr->vs<<endl);
                        return FoundUndecidedPhi;
                    }
                }
            }
